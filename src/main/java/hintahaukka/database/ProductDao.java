...
public class ProductDao {
    ...
    public Product add(String ean, String name, String schemaName) throws URISyntaxException, SQLException {
        return database.executeQueryAndExpectOneResult("INSERT INTO " + schemaName + ".Product (ean, name) VALUES (?, ?) RETURNING id", statement -> {
    ...
    public Product findOne(String ean, String schemaName) throws URISyntaxException, SQLException {
        return database.executeQueryAndExpectOneResult("SELECT * FROM " + schemaName + ".Product WHERE ean = ?", statement -> {
            statement.setString(1, ean);
        }, resultSet -> 
    ...
    public Boolean updateName(String ean, String newName, String schemaName) throws URISyntaxException, SQLException {
        return database.executeQueryAndExpectOneResult("UPDATE " + schemaName + ".Product SET name = ? WHERE ean = ? RETURNING id", statement -> {
