...
public class StorePointsDao {
    ...
    public Boolean add(User user, Store store, int points, String schemaName) throws URISyntaxException, SQLException {
        return database.executeQueryAndExpectOneResult("INSERT INTO " + schemaName + ".StorePoints (user_id, store_id, points) VALUES (?, ?, ?) RETURNING id", statement -> {
    ...
    public StorePoints findOne(User user, Store store, String schemaName) throws URISyntaxException, SQLException {
        return database.executeQueryAndExpectOneResult("SELECT * FROM " + schemaName + ".StorePoints WHERE user_id = ? AND store_id = ?", statement -> {
            statement.setInt(1, user.getId());
            statement.setInt(2, store.getId()); 
        }, resultSet -> 
    ...
    public ArrayList<StorePoints> find10LargestForStore(Store store, String schemaName) throws URISyntaxException, SQLException {
        return database.executeQueryAndExpectMultipleResults("SELECT * FROM " + schemaName + ".StorePoints WHERE store_id = ? ORDER BY points DESC LIMIT 10", statement -> {
    ...
    public Boolean updatePoints(StorePoints storePoints, String schemaName) throws URISyntaxException, SQLException {
        return database.executeQueryAndExpectOneResult("UPDATE " + schemaName + ".StorePoints SET points = ? WHERE id = ? RETURNING id", statement -> {
